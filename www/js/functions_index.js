...
function readURL(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            $('.foto-preview:eq(' + fotos.length + ')').clone().appendTo( ".block-fotod" );
            $('.foto-preview:eq(' + fotos.length + ')').show();
            $('.foto-preview:eq(' + fotos.length + ')').css('background-image', 'url(' + e.target.result + ')');
            $('.foto-upload:eq(' + fotos.length + ')').clone().appendTo( "#upload" );;

            fotos.push(input);

            $('.block-menos:eq(' + fotos.length + ')').attr('onclick', 'retirarFoto(' + fotos.length + ')');
...
function carregarViagens(result) {
    ...
    for(let i = 0; i < viagens.length; i++) {
        ...
        $('.data-viagem:eq(' + parseInt(i+1) + ')').html('de ' + viagens[i].dataInicio + ' a ' + viagens[i].dataVolta);
        $('.viagem:eq(' + parseInt(i+1) + ')').attr('onclick', 'modificarViagem(' + JSON.stringify(viagens[i]) + ')');
...
function carregarPecas(result) {
    ...
    for(let i = 0; i < pecas.length; i++) {
        ...
        $('.pecas-start-' + contador + ':eq(0)').clone().appendTo( '#coluna-pecas-' + contador );
        $('.pecas-start-' + contador + ':eq(' + colunas[contador] + ')').show();
        $('.peca-peca-' + contador + ':eq(' + colunas[contador] + ')').css('background-image', 'url(uploads/' + fotosPecas[0] + ')');
        $('.escrito-peca-' + contador + ':eq(' + colunas[contador] + ')').html(pecas[i].marca);
...
function categoriaPecasLook(categoria) {
    ...
    .then((result) => {
        ...
        for(let i = 0; i < pecas.length; i++) {
            ...
            $('.peca-escolha-look:eq(0)').clone().appendTo('#pecas-escolher-looks');
            $('.peca-escolha-look:eq(' + classIndex + ')').show();
            $('.peca-escolha-look:eq(' + classIndex + ')').css('background-image', 'url("uploads/' + fotosPecas[0] + '")');
            $('.peca-escolha-look:eq(' + classIndex + ')').attr('onclick', 'adicionarPecaLook(' + JSON.stringify(pecas[i]) + ')');
...
function adicionarPecaLook(peca) {
    ...
    if(pecasLook.length > 0) {
        ...
        for(let i = 0; i < pecasLook.length; i++) {
            ...
            $('.peca-escolhida-look:eq(0)').clone().appendTo('#pecas-escolhidas-look');
            $('.peca-escolhida-look:eq(' + classIndex + ')').show();
            $('.peca-escolhida-look:eq(' + classIndex + ')').css('background-image', 'url("uploads/' + fotosPecas[0] + '")');
            $('.peca-escolhida-look:eq(' + classIndex + ')').attr('onclick', 'removerPecaLook(' + i + ')');
...
function modificarViagem(viagem) {
    ...
    $('#mod-viagem-form').css({ display: 'flex', transition: 'transform 500ms ease 0s, opacity 200ms ease 0s', opacity: 1, transform: 'translateX(0px) translateY(0px)' })
...
function gerarCalendario(mes, ano) {
    ...
    for(let i = 0; i < 7; i++) {
        ...
        $('.block-dia:eq(0)').clone().appendTo('.block-semana1:eq(0)');
        $('.block-dia:eq(' + parseInt(i + 1) + ')').show();

        $('.nome-evento:eq(' + parseInt(i + 1) + ')').html('');

        if(i < diaSemana) {

            $('.data-evento:eq(' + parseInt(i + 1) + ')').html('')

        } else {

            $('.nome-evento:eq(' + parseInt(i + 1) + ')').html('SEM EVENTO');
            $('.data-evento:eq(' + parseInt(i + 1) + ')').html(diasCount);
            diasCount++;
